		...
		node.instance.CommandClass['COMMAND_CLASS_BASIC'].on('report', (command, report) => {
			...
			if (command.name === "BASIC_SET") {
				...
				if (singlePress === true) {
					...
					if (report.Value === 255) {
						Homey.manager('flow').triggerDevice('TZ36D_s2_single_on', null, null, node.device_data);
					}
					
					// Trigger Sinlge click off flow card
					else
					if (report.Value === 0) {
						Homey.manager('flow').triggerDevice('TZ36D_s2_single_off', null, null, node.device_data);
					}
				}
				...
				if (singlePress === false) {
					...
					if (report.Value === 255) {
						Homey.manager('flow').triggerDevice('TZ36D_s2_double_on', null, null, node.device_data);
					}
					
					// Trigger Double click off flow card
					else
					if (report.Value === 0) {
						Homey.manager('flow').triggerDevice('TZ36D_s2_double_off', null, null, node.device_data);
					}
				}
			}
		});
