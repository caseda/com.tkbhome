...
// TZ36S: N/A
// TZ36D: http://www.tkbhome.com/?cn-p-d-264.html
// TZ56S: http://www.tkbhome.com/?cn-p-d-267.html
// TZ56D: http://www.tkbhome.com/?cn-p-d-268.html
			...
			node.instance.CommandClass.COMMAND_CLASS_BASIC.on('report', (command, report) => {
				...
				if (command.name === 'BASIC_SET') {
					...
					if (singlePress[token] === true) {
						...
						if (report.Value === 255) Homey.manager('flow').triggerDevice('TZ36D_s2_single_on', null, null, node.device_data);

						// Trigger Single click off flow card
						if (report.Value === 0) Homey.manager('flow').triggerDevice('TZ36D_s2_single_off', null, null, node.device_data);
					}

					// Double press registered
					else if (singlePress[token] === false) {
						// Trigger Double click on flow card
						if (report.Value === 255) Homey.manager('flow').triggerDevice('TZ36D_s2_double_on', null, null, node.device_data);

						// Trigger Double click off flow card
						if (report.Value === 0) Homey.manager('flow').triggerDevice('TZ36D_s2_double_off', null, null, node.device_data);
					}
				}
			});
		}
